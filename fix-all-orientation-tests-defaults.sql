...
-- Script complet pour corriger toutes les valeurs par défaut de orientation_tests

SELECT '=== CORRECTION COMPLÈTE DES VALEURS PAR DÉFAUT ===' AS Info;

-- 1. Supprimer test_date si elle existe (redondante avec test_started_at)
SET @sql = (SELECT IF(
...
-- 2. Ajouter des valeurs par défaut aux colonnes NOT NULL
...
-- 3. Ajouter les colonnes manquantes avec valeurs par défaut
SET @sql = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS
     WHERE TABLE_SCHEMA = 'diravenir'
     AND TABLE_NAME = 'orientation_tests'
...
SELECT '=== TEST D\'INSERTION COMPLET ===' AS Info;
...
WHERE student_email = 'test.complete@example.com'
ORDER BY id DESC 
LIMIT 1;

-- Supprimer le test de test
DELETE FROM orientation_tests WHERE student_email = 'test.complete@example.com';

SELECT '✅ Toutes les corrections appliquées avec succès à orientation_tests' AS Status;
