...
/**
 * Tests unitaires complets pour la stratégie de matching simple.
 * 
 ...
 * - Calculs de correspondance simple corrects
 */
@DisplayName("Tests complets de la stratégie de matching simple")
class SimpleMatchingStrategyTest {
    
    private SimpleMatchingStrategy strategy;
    private UserProfileDTO userProfile;
    private MajorProfileDTO majorProfile;
    
    @BeforeEach
    void setUp() {
        strategy = new SimpleMatchingStrategy();
    ...
    class BasicTests {
        ...
        void testGetAlgorithmName() {
            ...
            assertEquals("Simple Matching Strategy", algorithmName);
        }
        ...
        void testGetPriority() {
            ...
            assertEquals(3, priority); // Priorité basse selon l'implémentation
        }
    ...
    class ScoreValidationTests {
        ...
        void testExecuteReturnsValidScore() {
            ...
            // Pour ce profil utilisateur et cette majeure, on s'attend à un score moyen
            // car la stratégie simple est moins sophistiquée
            assertTrue(score > 0.3, "Le score devrait être > 30% pour ce profil");
        }
    ...
    @Nested
    @DisplayName("Tests de calcul de correspondance simple")
    class SimpleMatchingTests {
        
        @Test
        @DisplayName("La stratégie doit calculer correctement la correspondance simple")
        void testSimpleMatchingCalculation() {
        ...
        void testIdenticalProfiles() {
            ...
            // Le score doit être élevé (proche de 1.0) pour des profils identiques
            assertTrue(score > 0.8, 
                "Des profils identiques doivent avoir un score élevé (> 80%)");
        }
        ...
        void testVeryDifferentProfiles() {
            ...
            assertTrue(score < 0.6, 
                "Des profils très différents doivent avoir un score plus faible (< 60%)");
        }
        
        @Test
        @DisplayName("La stratégie doit normaliser correctement les scores")
    ...
    class ErrorHandlingTests {
        ...
        @Test
        @DisplayName("La stratégie doit gérer les profils vides")
        void testExecuteWithEmptyProfiles() {
            // Création de profils avec des scores à 0
            UserProfileDTO emptyUser = createEmptyUserProfile();
            MajorProfileDTO emptyMajor = createEmptyMajorProfile();
            
            assertDoesNotThrow(() -> {
                double score = strategy.execute(emptyUser, emptyMajor);
                // La stratégie doit gérer gracieusement les profils vides
                assertTrue(score >= 0.0 && score <= 1.0);
            });
        }
    }
    ...
    class DifferentMajorTypesTests {
        ...
        void testScoreSensitivity() {
            ...
            assertTrue(Math.abs(score1 - score2) < 0.15, 
    ...
    /**
     * Crée un profil majeure vide avec tous les scores à 0
     */
