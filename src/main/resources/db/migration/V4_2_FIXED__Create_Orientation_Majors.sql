-- Migration V4.2 FIXED: Création des profils idéaux des majeures pour l'orientation
-- Date: 2025-01-27
-- Objectif: Séparer les profils idéaux (orientation) des programmes universitaires (program)
-- VERSION CORRIGÉE: Gestion correcte de l'ordre des opérations

-- Étape 1: Vérifier et supprimer les tables existantes dans le bon ordre
-- (supprimer d'abord les tables qui ont des contraintes de clé étrangère)

-- Supprimer la table major_program_mapping si elle existe (dépend de ideal_profiles)
DROP TABLE IF EXISTS major_program_mapping;

-- Supprimer la table ideal_profiles si elle existe (dépend de orientation_majors)
...
-- Étape 2: Créer la table des majeures d'orientation en premier
...
-- Étape 3: Créer la table des profils idéaux avec contrainte de clé étrangère
...
-- Étape 4: Créer la table de mapping avec contraintes de clé étrangère
...
-- Étape 6: Insérer les majeures d'orientation (50 majeures)
...
-- Étape 7: Vérifier que les majeures ont été insérées correctement
SELECT 'VÉRIFICATION: Majeures insérées' as status, COUNT(*) as total_majeures FROM orientation_majors;

-- Étape 8: Insérer les profils idéaux pour les 10 majeures principales
-- Génie Civil (ID: 1)
...
-- Étape 9: Vérifier que les profils idéaux ont été insérés correctement
SELECT 'VÉRIFICATION: Profils idéaux insérés' as status, COUNT(*) as total_profils FROM ideal_profiles;

-- Étape 10: Insérer les mappings d'exemple
...
-- Étape 11: Vérifier que les mappings ont été insérés correctement
SELECT 'VÉRIFICATION: Mappings insérés' as status, COUNT(*) as total_mappings FROM major_program_mapping;

-- Étape 12: Vérification finale de la cohérence
SELECT 
...
SELECT 'Migration V4.2 FIXED: Profils idéaux des majeures d''orientation créés avec succès' AS status;
