...
import com.dira.diravenir1.Repository.UniversiteRepository;
...
public class ProgramUploadController {
                ...
                for (int sheetIndex = 0; sheetIndex < totalSheets; sheetIndex++) {
                    ...
                    for (int i = 1; i <= sheet.getLastRowNum(); i++) {
                        ...
                        try {
                            ...
                            program.setCreatedAt(currentDate);
                            program.setUpdatedAt(currentDate);
    ...
    private Destination determineDestinationFromSheetName(String sheetName) {
        ...
        if (normalizedName.contains("chine") || normalizedName.contains("china")) {
            return destinationRepository.findByNomContainingIgnoreCase("Chine")
                    .orElseGet(() -> createDestinationIfNotExists("Chine", "Programmes d'études en Chine"));
        } else if (normalizedName.contains("chypre") || normalizedName.contains("cyprus")) {
            return destinationRepository.findByNomContainingIgnoreCase("Chypre")
                    .orElseGet(() -> createDestinationIfNotExists("Chypre", "Programmes d'études à Chypre"));
        }
    ...
    private Universite getOrCreateUniversite(String universityName, Destination destination) {
        ...
        return universiteRepository.findByNomContainingIgnoreCase(universityName.trim())
                .orElseGet(() -> createUniversiteIfNotExists(universityName.trim(), destination));
    }
    ...
    private Universite createUniversiteIfNotExists(String nom, Destination destination) {
        ...
        // Définir les dates automatiquement
        String currentDate = java.time.LocalDate.now().toString();
        universite.setCreatedAt(currentDate);
        universite.setUpdatedAt(currentDate);
    ...
    private void validateUniversiteBeforeSave(Universite universite) {
        ...
        if (universite.getCreatedAt() == null || universite.getCreatedAt().trim().isEmpty()) {
            throw new IllegalArgumentException("Date de création est obligatoire");
        }
        if (universite.getUpdatedAt() == null || universite.getUpdatedAt().trim().isEmpty()) {
    ...
    private void validateProgramBeforeSave(Program program) {
        ...
        if (program.getCreatedAt() == null || program.getCreatedAt().trim().isEmpty()) {
            throw new IllegalArgumentException("Date de création est obligatoire");
        }
        if (program.getUpdatedAt() == null || program.getUpdatedAt().trim().isEmpty()) {
            throw new IllegalArgumentException("Date de mise à jour est obligatoire");
        }
    }
