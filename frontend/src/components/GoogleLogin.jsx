import React from 'react';

const GoogleLogin = ({ onSuccess, onError, variant = "default", className = "" }) => {
  const handleGoogleLogin = () => {
    try {
      // Rediriger vers l'endpoint OAuth2 de Spring Security
      const googleAuthUrl = 'http://localhost:8084/oauth2/authorization/google';
      window.location.href = googleAuthUrl;
    } catch (error) {
      if (onError) onError(error);
    }
  };

  const baseClasses = `
    inline-flex items-center justify-center gap-3 px-6 py-3 
    font-medium text-gray-700 bg-white border border-gray-300 
    rounded-lg shadow-sm hover:bg-gray-50 hover:border-gray-400 
    focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 
    transition-all duration-200 ease-in-out transform hover:scale-105 
    active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed
  `;

  const variantClasses = {
    default: baseClasses,
    primary: `
      inline-flex items-center justify-center gap-3 px-6 py-3 
      font-medium text-white bg-blue-600 border border-blue-600 
      rounded-lg shadow-sm hover:bg-blue-700 hover:border-blue-700 
      focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 
      transition-all duration-200 ease-in-out transform hover:scale-105 
      active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed
    `,
    outline: `
      inline-flex items-center justify-center gap-3 px-6 py-3 
      font-medium text-blue-600 bg-transparent border-2 border-blue-600 
      rounded-lg hover:bg-blue-50 focus:outline-none focus:ring-2 
      focus:ring-blue-500 focus:ring-offset-2 transition-all duration-200 
      ease-in-out transform hover:scale-105 active:scale-95 
      disabled:opacity-50 disabled:cursor-not-allowed
    `
  };

  return (
    <div className={`google-login-container ${className}`}>
      <button
        onClick={handleGoogleLogin}
        className={variantClasses[variant]}
        type="button"
      >
        {/* Icône Google moderne */}
        <svg 
          width="20" 
          height="20" 
          viewBox="0 0 24 24" 
          className="flex-shrink-0"
          aria-hidden="true"
        >
          <path
            fill="#4285F4"
            d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
          />
          <path
            fill="#34A853"
            d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
          />
          <path
            fill="#FBBC05"
            d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
          />
          <path
            fill="#EA4335"
            d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
          />
        </svg>
        
        {/* Texte du bouton */}
        <span className="font-medium">
          {variant === 'primary' ? 'Se connecter avec Google' : 'Continuer avec Google'}
        </span>
        
        {/* Icône de flèche pour indiquer l'action */}
        <svg 
          width="16" 
          height="16" 
          viewBox="0 0 24 24" 
          fill="none" 
          stroke="currentColor" 
          strokeWidth="2" 
          strokeLinecap="round" 
          strokeLinejoin="round"
          className="flex-shrink-0 opacity-60"
        >
          <path d="M5 12h14M12 5l7 7-7 7"/>
        </svg>
      </button>
      
      {/* Message d'information */}
      <p className="text-xs text-gray-500 mt-2 text-center">
        Connexion sécurisée via Google
      </p>
    </div>
  );
};

export default GoogleLogin; 